...
Manager::Manager() {
    ...
    if (std::filesystem::exists("Manager_List"))
        List.get_info(cnt, 1);
    else
        cnt = 0;
    BookStore.Init("BookStore");
    ISBN_Store.Init("ISBN_Storage");
    Keyword_Store.Init("Keyword_Storage");
    BookName_Store.Init("BookName_Storage");
    Author_Store.Init("Author_Storage");
}

Manager::~Manager() { List.write_info(cnt, 1); }

bool Manager::check_ISBN(string &ISBN) {
    if (ISBN.size() > 20 || ISBN.size() == 0)
        throw 0;
    for (auto c : ISBN)
        if (!checkChar(c))
            throw 0;
...
bool Manager::check_Name(string &Name) {
    int len = Name.size();
    if (Name.size() > 62 || Name.size() < 3) throw 0;
    if (Name[0] != '"' || Name[len - 1] != '"') throw 0;
    for (int i = 1; i < len - 1; ++i)
        if (!checkChar(Name[i]) || Name[i] == '"')
            throw 0;
    return true;
}

bool Manager::check_Keyword(string &Keyword) {
...
bool Manager::checkDiv(string &Keyword) {
    int len = Keyword.size();
    for (int i = 1; i < len - 1; ++i)
        if (Keyword[i] == '|')
            throw 0;
...
bool Manager::ISBN_cmp(const Book &A, const Book &B) { return A.ISBN < B.ISBN; }

bool Manager::find_ID(int id, Book &obj) {
...
bool Manager::find_ISBN(string &ISBN, Book &obj) {
...
bool Manager::find_BookName(string &BookName, std::vector<Book> &obj) {
    ...
    for (int i = 0; i < obj.size(); ++i)
        find_ID(res[i], obj[i]);
    std::sort(obj.begin(), obj.end(), ISBN_cmp);
    return true;
}

bool Manager::find_Keyword(string &Keyword, std::vector<Book> &obj) {
...
bool Manager::find_Author(string &Author, std::vector<Book> &obj) {
...
void Manager::print(std::vector<Book> res) {
    if (res.size() == 0)
        std::cout << '\n';
    for (auto vec : res)
        std::cout << vec;
...
string Manager::getOpt(string msg) {
    string st = "";
    for (auto c : msg) {
        if (c == '=') return st;
        else st += c;
...
string Manager::getWord(string msg) {
    string st = "";
    int pos = -1;
...
void Manager::show(Read &a) {
    ...
    if (a.length() == 0) {
        ...
        for (int i = 1; i <= cnt; ++i)
            find_ID(i, res[i - 1]);
        std::sort(res.begin(), res.end(), ISBN_cmp);
        print(res);
        return;
    }
...
void Manager::buy(Read &a) {
    ...
    std::cout << std::fixed << std::setprecision(2)
              << 1.0 * quantity * target.Price << '\t' << std::fixed
              << std::setprecision(0) << '\n';
...
void Manager::check_modify(Read &a) {
    User cur = Stack.getTop();
    if (cur.Privilege < 3) throw 0;
