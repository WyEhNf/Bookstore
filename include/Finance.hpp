...
public:
  ...
  Finance(int ID, double inCome, double outCome) {
    id = ID;
    income = inCome;
    outcome = outCome;
  }
  bool operator==(const Finance &A) const { return id == A.id; }
  bool operator!=(const Finance &A) const { return id != A.id; }
  bool operator<(const Finance &A) const { return id < A.id; }
  bool operator<=(const Finance &A) const { return id <= A.id; }
  bool operator>(const Finance &A) const { return !(*this <= A); }
  bool operator>=(const Finance &A) const { return !(*this < A); }
...
std::ostream &operator<<(std::ostream &out, const Finance &now) {
  out << std::fixed << std::setprecision(2) << "+ " << now.income << " - "
      << now.outcome << std::fixed << std::setprecision(0);
  return out;
}
